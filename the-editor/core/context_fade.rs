...
impl ContextAnalyzer {
    ...
    fn find_node_at_position(node: tree_house::tree_sitter::Node, byte_pos: u32) -> Option<(u32, u32)> {
    ...
    /// Find all usage contexts of an identifier (statements/expressions containing it)
    fn find_usage_contexts(
        ...
        eprintln!("[FADE DEBUG] Found {} relevant ranges", relevant.ranges.len());
        for (i, range) in relevant.ranges.iter().enumerate() {
            eprintln!("[FADE DEBUG]   Range {}: {}..{}", i, range.from(), range.to());
        }
    }
    
    /// Recursively collect usage contexts (statements/expressions) containing the identifier
    fn collect_usage_contexts(
        ...
        if is_match {
            eprintln!("[FADE DEBUG] Found identifier match at {}..{}", node.start_byte(), node.end_byte());
            ...
            if let Some(context_node) = Self::find_containing_context(node) {
                ...
                eprintln!("[FADE DEBUG]   Context node kind: {}, range: {}..{}", context_node.kind(), start_char, end_char);
    ...
    fn find_containing_context(node: tree_house::tree_sitter::Node) -> Option<tree_house::tree_sitter::Node> {
        // Statement/expression kinds that represent meaningful contexts
        // These are actual Rust tree-sitter node types
        // We exclude overly broad items like function_item to avoid highlighting entire functions
    ...
    pub fn extract_identifiers_in_range(&self, text: RopeSlice, start: usize, end: usize) -> Vec<String> {
