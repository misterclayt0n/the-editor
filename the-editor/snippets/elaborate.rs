...
impl Snippet {
  ...
  fn elaborate_placeholder(
    ...
    if idx == LAST_TABSTOP_IDX && !default.is_empty() {
      // Older versions of clangd for example may send a snippet like
      // `${0:placeholder}` which is considered by VSCode to be a misuse of the
      // `$0` tabstop.
      println!(
        "Discarding placeholder text for the `$0` tabstop ({default:?}). The `$0` tabstop \
         signifies the final cursor position and should not include placeholder text."
      );
      default.clear();
    }
...
impl Transform {
  fn new(transform: parser::Transform) -> Option<Transform> {
    ...
    if invalid_config {
      println!("invalid transform configuration characters {transform:?}");
    }
    let regex = match RegexBuilder::new().syntax(config).build(&transform.regex) {
      Ok(regex) => regex,
      Err(err) => {
        println!("invalid transform {err} {transform:?}");
        return None;
      },
    };
    Some(Transform {
