...
pub enum LineNumber {
  ...
  /// If focused and in normal/select mode, show relative line number to the
  /// primary cursor. If unfocused or in insert mode, show absolute line
  /// number.
  Relative,
}
...
impl GutterType {
  pub fn style<'doc>(
    ...
    match self {
      GutterType::Diagnostics => diagnostics_or_breakpoints(editor, doc, view, theme, is_focused),
  ...
  if let Some(diff_handle) = doc.diff_handle() {
    ...
    Box::new(
      move |line: usize, _selected: bool, first_visual_line: bool, out: &mut String| {
        ...
        // these technically do not have a range of lines to highlight
        // (`hunk.after.start == hunk.after.end`). However we still want to
        // display these hunks correctly we must not yet skip to the next hunk here
...
//   Box::new(
//     move |line: usize, _selected: bool, first_visual_line: bool, out: &mut
// String| {       if !first_visual_line {
//         return None;
//       }
...
// breakpoint.log_message.is_some() {         error.
// underline_style(UnderlineStyle::Line)       } else if
// breakpoint.condition.is_some() {         error
...
impl std::str::FromStr for GutterType {
  ...
  fn from_str(s: &str) -> Result<Self, Self::Err> {
    match s.to_lowercase().as_str() {
      ...
      "diff" => Ok(Self::Diff),
      _ => {
        anyhow::bail!("Gutter type can only be `diagnostics`, `spacer`, `line-numbers` or `diff`.")
      },
    }
  }
}
...
impl Editor {
  pub fn new(
    ...
    let language_servers = crate::lsp::Registry::new(syn_loader.clone());
    ...
    let editor = Self {
      ...
      registers: Registers::new(Box::new(arc_swap::access::Map::new(
        Arc::clone(&config),
        |config: &EditorConfig| &config.clipboard_provider,
      ))),
      status_msg: None,
      autoinfo: None,
  ...
  /// relevant members.
  pub fn refresh_config(&mut self, old_config: &EditorConfig) {
  ...
  pub fn unset_theme_preview(&mut self) {
    ...
    // None likely occurs when the user types ":theme" and then exits before
    // previewing
  }
  ...
  pub fn move_path(&mut self, old_path: &Path, new_path: &Path) -> io::Result<()> {
    ...
    for language_server in language_servers {
      ...
      let edit = match block_on(request) {
      ...
      self
        ...
        .filter_map(|(lang, client)| {
          match client {
            ...
            Err(err) => {
              if let crate::lsp::Error::ExecutableNotFound(err) = err {
  ...
  fn _refresh(&mut self) {
    ...
    // Reset the inlay hints annotations *before* updating the views, that way we
    // ensure they will disappear during the `.sync_change(doc)` call below.
    //
    // We can't simply check this config when rendering because inlay hints are only
    // parts of the possible annotations, and others could still be active, so
    // we need to selectively drop the inlay hints.
  ...
  pub fn close_document(&mut self, doc_id: DocumentId, force: bool) -> Result<(), CloseError> {
    ...
    // views. We don't want to close the editor just for a simple buffer close,
    // so we need to create a new view containing either an existing document,
    // or a brand new document.
    ...
    the_editor_event::dispatch(DocumentDidClose { editor: self, doc });
  ...
  pub fn doc_diagnostics<'a>(
    language_servers: &'a crate::lsp::Registry,
    diagnostics: &'a Diagnostics,
    document: &Document,
  ) -> impl Iterator<Item = crate::core::diagnostics::Diagnostic> + 'a {
    Editor::doc_diagnostics_with_filter(language_servers, diagnostics, document, |_, _| true)
  }

  /// Returns all supported diagnostics for the document
  /// filtered by `filter` which is invocated with the raw `lsp::Diagnostic` and
  /// the language server id it came from
  pub fn doc_diagnostics_with_filter<'a>(
    language_servers: &'a crate::lsp::Registry,
    diagnostics: &'a Diagnostics,
    document: &Document,
    filter: impl Fn(&the_editor_lsp_types::types::Diagnostic, &DiagnosticProvider) -> bool + 'a,
  ) -> impl Iterator<Item = crate::core::diagnostics::Diagnostic> + 'a {
  ...
  pub async fn wait_event(&mut self) -> EditorEvent {
    // the loop only runs once or twice and would be better implemented with a
    // recursion + const generic however due to limitations with async functions
    // that can not be implemented right now
    loop {
      tokio::select! {
          biased;
          ...
          _ = the_editor_event::redraw_requested() => {
    ...
    if doc.restore_cursor {
      ...
      let selection = doc.selection(view.id).clone().transform(|range| {
        ...
        if range.head > range.anchor {
          head = crate::core::grapheme::prev_grapheme_boundary(text, head);
        }
