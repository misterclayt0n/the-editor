...
//! However the RopeSlice references the original rope and therefore forms a
//! self-referential data structure. A transmute is used to change the lifetime
...
impl InternedRopeLines {
  ...
  /// is therefore significantly faster than `update_diff_base` when only the
  /// document changes.
  ...
  fn update_diff_base_impl(&mut self) {
    // Safety: This transmute is safe because it only transmutes a lifetime, which
    // has no effect. The backing storage for the RopeSlices referred to by the
    // lifetime is stored in `self.diff_base`. Therefore as long as
    // `self.diff_base` is not dropped/replaced this memory remains valid.
    // `self.diff_base` is only changed in `self.update_diff_base`, which clears the
    // interner. When the interned lines are exposed to consumer in
    // `self.diff_input`, the lifetime is bounded to a reference to self.
    // That means that on calls to update there exist no references to
    // `self.interned`.
    let before = self
      .diff_base
      .lines()
  ...
  fn update_doc_impl(&mut self) {
    // Safety: This transmute is save because it only transmutes a lifetime, which
    // has no effect. The backing storage for the RopeSlices referred to by the
    // lifetime is stored in `self.doc`. Therefore as long as `self.doc` is not
    // dropped/replaced this memory remains valid. `self.doc` is only changed in
    // `self.update_doc`, which clears the interner. When the interned lines are
    // exposed to consumer in `self.diff_input`, the lifetime is bounded to a
    // reference to self. That means that on calls to update there exist no
    // references to `self.interned`.
    let after = self
      .doc
      .lines()
      .map(|line: RopeSlice| -> RopeSlice<'static> { unsafe { transmute(line) } });
  ...
  /// If `diff_base` or `doc` is so large that performing a diff could slow the
  /// editor this function returns `None`.
