...
impl IndentStyle {
  ...
  ///
  /// For example, passing `"    "` (four spaces) will create
...
#[inline]
fn is_indent_whitespace(ch: char) -> bool {
  WhitespaceProperties::of(ch).is_some()
}

#[inline]
fn text_width(text: RopeSlice, tab_width: usize) -> usize {
  let mut col = 0;
  for grapheme in text.graphemes() {
    if grapheme == "\t" {
      col += tab_width_at(col, tab_width as u16);
    } else {
      col += grapheme_width(&Cow::from(grapheme));
    }
  }
  col
}
...
#[inline]
fn whitespace_width(text: &str, tab_width: usize) -> usize {
  let mut col = 0;
...
fn whitespace_prefix_for_width(text: &str, target: usize, tab_width: usize) -> usize {
  let mut col = 0;
  let mut byte_end = 0;
    ...
    'outer: for line in document_text.lines().take(1000) {
      ...
      for c in c_iter {
        match c {
          ...
          c if is_indent_whitespace(c) => count_is_done = true,
...
/// To determine indentation of a newly inserted line, figure out the
/// indentation at the last col of the previous line.
...
/// Create a string of tabs & spaces that has the same visual width as the given
/// RopeSlice (independent of the tab width).
...
/// This function does not change the actual indentation width, just the
...
pub fn normalize_indentation(
  ...
  let off = text_width(prefix, tab_width);
  let mut len = 0;
...
fn add_indent_level(
  ...
  if added_indent_level >= 0 {
    ...
    // the chars of `base_indent` in reverse until we have the desired indent
    // reduction, instead we iterate over them twice in forward direction.
    let base_indent_width = whitespace_width(&base_indent, tab_width);
    let target_indent_width = base_indent_width
      .saturating_sub((-added_indent_level) as usize * indent_style.indent_width(tab_width));
    let byte_end_idx = whitespace_prefix_for_width(&base_indent, target_indent_width, tab_width);
...
fn is_first_in_line(node: &Node, text: RopeSlice, new_line_byte_pos: Option<u32>) -> bool {
  ...
  text
    .byte_slice(line_start_byte_pos as usize..node.start_byte() as usize)
    .chars()
...
/// - Successively add indent captures to get the (added) indent from a single
///   line
/// - Successively add the indent results for each line The string that this
///   indentation defines starts with the string contained in the align field
///   (unless it is None), followed by:
/// - max(0, indent - outdent) tabs, if tabs are used for indentation
/// - max(0, indent - outdent)*indent_width spaces, if spaces are used for
///   indentation
...
impl<'a> Indentation<'a> {
  ...
  /// Indent should always be added starting from the bottom (or equivalently,
  /// the innermost tree-sitter node).
  ...
  /// Only captures that apply to the same line should be added together in this
  /// way (otherwise use `add_line`) and the captures should be added starting
  /// from the innermost tree-sitter node (currently this only matters
  ...
  fn net_indent(&self) -> isize {
    (self.indent + self.indent_always) as isize - ((self.outdent + self.outdent_always) as isize)
  }
  /// Convert `self` into a string, taking into account the computed and actual
  /// indentation of some other line.
  fn relative_indent(
    &self,
    ...
    if self.align == other_computed_indent.align {
      // If self and baseline are either not aligned to anything or both aligned the
      // same way, we can simply take `other_leading_whitespace` and add some
      // indent / outdent to it (in the second case, the alignment should
      // already be accounted for in `other_leading_whitespace`).
      ...
      // If the alignment of both lines is different, we cannot compare their
      // indentation in any meaningful way
      None
    }
  }
...
fn get_node_end_line(text: RopeSlice, node: &Node, new_line_byte_pos: Option<u32>) -> usize {
  ...
  // Adjust for the new line that will be inserted (with a strict inequality since
...
fn query_indents<'a>(
  ...
  while let Some(m) = cursor.next_match() {
    ...
    // They cannot be added to indent_captures immediately since they may depend on
    // other captures (such as an @anchor).
    ...
    for matched_node in m.matched_nodes() {
      ...
      let capture_type = if capture == query.indent_capture {
        ...
        if anchor.is_some() {
          println!("Invalid indent query: Encountered more than one @anchor in the same match.")
        } else {
        ...
        // Ignore any unknown captures (these may be needed for predicates such as
        // #match?)
        continue;
      };
    ...
    for (node_id, mut capture) in added_indent_captures {
      ...
      if let IndentCaptureType::Align(_) = capture.capture_type {
        let Some(anchor) = anchor else {
          println!("Invalid indent query: @align requires an accompanying @anchor.");
          continue;
        };
        let line = text.byte_to_line(anchor.start_byte() as usize);
        let line_start = text.line_to_byte(line);
        capture.capture_type =
          IndentCaptureType::Align(text.byte_slice(line_start..anchor.start_byte() as usize));
      }
      indent_captures
        .entry(node_id)
        .or_insert_with(|| Vec::with_capacity(1))
...
/// Handle extend queries. deepest_preceding is the deepest descendant of node
/// that directly precedes the cursor position. Any ancestor of
/// deepest_preceding which is also a descendant of node may be "extended". In
/// that case, node will be updated, so that the indent computation starts with
/// the correct syntax node.
fn extend_nodes<'a>(
  node: &mut Node<'a>,
      ...
      for capture in captures {
        match capture {
          ...
          ExtendCapture::Extend => {
            ...
            // - the line that the cursor is on is more indented than the first line of the
            //   node
            if text.byte_to_line(deepest_preceding.end_byte() as usize) == line {
              extend_node = true;
            } else {
              let cursor_indent = indent_level_for_line(text.line(line), tab_width, indent_width);
              let node_indent = indent_level_for_line(
                text.line(text.byte_to_line(deepest_preceding.start_byte() as usize)),
                tab_width,
                indent_width,
              );
...
/// - The node from which to start the query (this is non-trivial due to
///   `@extend` captures)
...
fn init_indent_query<'a, 'b>(
  ...
  // Check for extend captures, potentially changing the node that the indent
...
///
/// - To get the correct indentation for an existing line (new_line=false), not
///   necessarily equal to the current indentation.
///   - In this case, pos should be inside the first tree-sitter node on that
///     line. In most cases, this can just be the first non-whitespace on that
///     line.
///   - To get the indentation for a new line (new_line=true). This behaves like
///     the first usecase if the part of the current line after pos were moved
///     to a new line.
///
/// The indentation is determined by traversing all the tree-sitter nodes
/// containing the position. Each of these nodes produces some [Indentation]
/// for:
///
/// - The line of the (beginning of the) node. This is defined by the scope
///   `all` if this is the first node on its line.
/// - The line after the node. This is defined by:
///   - The scope `tail`.
///   - The scope `all` if this node is not the first node on its line.
///
/// Intuitively, `all` applies to everything contained in this node while `tail`
/// applies to everything except for the first line of the node. The indents
/// from different nodes for the same line are then combined. The result
/// [Indentation] is simply the sum of the [Indentation] for all lines.
///
/// Specifying which line exactly an [Indentation] applies to is important
/// because indents on the same line combine differently than indents on
/// different lines: ```ignore
/// some_function(|| {
///     // Both the function parameters as well as the contained block should be
/// indented.     // Because they are on the same line, this only yields one
/// indent level });
/// ```text
/// some_function(
///     param1,
///     || {
///         // Here we get 2 indent levels because the 'parameters' and the
///         // 'block' node begin on different lines
///     },
/// );
/// ```
#[allow(clippy::too_many_arguments)]
pub fn treesitter_indent_for_pos<'a>(
  ...
  // We always keep track of all the indent changes on one line, in order to only
  // indent once even if there are multiple "indent" nodes on the same line
...
/// This is done either using treesitter, or if that's not available by copying
/// the indentation from the current line
#[allow(clippy::too_many_arguments)]
pub fn indent_for_newline(
  ...
  // Fallback in case we either don't have indent queries or they failed for some
  // reason
...
mod test {
  ...
  fn test_relative_indent() {
    ...
    // Check that relative and absolute indentation computation are the same when
    // the line we compare to is indented as we expect.
