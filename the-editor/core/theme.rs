...
impl Loader {
  ...
  ///
  /// The provided directories should be ordered from highest to lowest
  /// priority. The directories will have their "themes" subdirectory
  /// searched.
  ...
  /// Loads a theme searching directories in priority order, returning any
  /// warnings
  ...
  ///
  /// It is possible for one file to inherit from another file with the same
  /// name so long as the second file is in a themes directory with lower
  /// priority. However, it is not recommended that users do this as it will
  /// make tracing errors more difficult.
  fn load_theme(&self, name: &str, visited_paths: &mut HashSet<PathBuf>) -> Result<Value> {
    ...
    let theme_toml = if let Some(parent_theme_name) = inherits {
      let parent_theme_name = parent_theme_name
        .as_str()
        .ok_or_else(|| anyhow!("Expected 'inherits' to be a string: {}", parent_theme_name))?;
...
impl Theme {
  /// To allow `Highlight` to represent arbitrary RGB colors without turning it
  /// into an enum, we interpret the last 256^3 numbers as RGB.
  ...
  pub fn scope(&self, highlight: Highlight) -> &str {
    self.scopes.get(highlight.idx()).map_or("", |s| s.as_str())
  }
