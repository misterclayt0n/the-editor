...
pub struct RelatedFullDocumentDiagnosticReport {
  ...
  ///
  /// This information is useful in programming languages where code in a file A
  /// can generate diagnostics in a file B which A depends on. An example of
  /// such a language is C/C++ where macro definitions in a file `a.cpp`
  /// result in errors in a header file `b.hpp`.
  ///
  /// @since 3.17.0
  #[serde(with = "crate::types::url_map")]
  #[serde(skip_serializing_if = "Option::is_none")]
  #[serde(default)]
