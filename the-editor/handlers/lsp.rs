...
pub enum ApplyEditErrorKind {
  DocumentChanged,
  FileNotFound,
  InvalidUrl(crate::core::uri::UrlConversionError),
...
impl From<crate::core::uri::UrlConversionError> for ApplyEditErrorKind {
  fn from(err: crate::core::uri::UrlConversionError) -> Self {
...
impl Editor {
  ...
  fn apply_document_resource_op(&mut self, op: &ResourceOp) -> Result<(), ApplyEditErrorKind> {
    use std::fs;
  ...
  pub fn handle_lsp_diagnostics(
    ...
    {
      log::info!(
        "Version ({version}) is out of date for {uri:?} (expected ({})), dropping \
      ...
      .and_then(|doc| Some((doc.language_config()?, self.diagnostics.get(&uri)?)))
    {
      if !lang_conf.persistent_diagnostic_sources.is_empty() {
        ...
        // Note: The `lsp::DiagnosticSeverity` enum is already defined in decreasing
        // order
      ...
      for source in &lang_conf.persistent_diagnostic_sources {
        ...
        let old_diagnostics = old_diagnostics
          .iter()
          .filter(|(d, d_provider)| d_provider == provider && d.source.as_ref() == Some(source))
          .map(|(d, _)| d);
    ...
    // Insert the original lsp::Diagnostics here because we may have no open
    // document for diagnostic message and so we can't calculate the exact
    // position. When using them later in the diagnostics picker, we calculate
    // them on-demand.
    let diagnostics = match self.diagnostics.entry(uri) {
      Entry::Occupied(o) => {
        let current_diagnostics = o.into_mut();
        // there may entries of other language servers, which is why we can't overwrite
        // the whole entry
    ...
    if let Some(doc) = doc {
      let diagnostic_of_language_server_and_not_in_unchanged_sources =
        |diagnostic: &Diagnostic, d_provider: &DiagnosticProvider| {
          d_provider == provider
            && diagnostic
              .source
              .as_ref()
      ...
      let doc = doc.id();
      the_editor_event::dispatch(DiagnosticsDidChange { editor: self, doc });
    }
  }

  pub fn execute_lsp_command(&mut self, command: Command, server_id: LanguageServerId) {
    // the command is executed on the server and communicated back
    // to the client asynchronously using workspace edits

    // Get the server and check if it supports commands
    let Some(server) = self.language_servers.get_by_id(server_id).cloned() else {
      self.set_error("Language server not found");
      return;
    };

    tokio::spawn(async move {
      let Some(future) = server.command(command) else {
        log::error!("Language server does not support executing commands");
        return;
      };
