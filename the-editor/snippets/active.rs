...
impl ActiveSnippet {
  ...
  // computes the primary idx adjusted for the number of cursors in the current
  // tabstop
  ...
  pub fn insert_subsnippet(mut self, snippet: RenderedSnippet) -> Option<Self> {
    ...
    {
      println!("number of subsnippets did not match, discarding outer snippet");
...
mod tests {
  ...
  fn tabstop_zero_with_placeholder() {
    // The `$0` tabstop should not have placeholder text. When we receive a snippet
    // like this (from older versions of clangd for example) we should discard
