...
if [[ ! -d "$PROJECT_DIR/ghostty" ]]; then
  echo "error: ghostty submodule is missing. Run ./scripts/setup_ghostty.sh first." >&2
  exit 1
fi
...
if [[ ! -f "$PROJECT_DIR/ghostty/include/ghostty.h" ]]; then
  echo "error: ghostty/include/ghostty.h is missing. Run ./scripts/setup_ghostty.sh first." >&2
  exit 1
fi
...
CACHE_ROOT="${THE_EDITOR_GHOSTTYKIT_CACHE_DIR:-$HOME/.cache/the-editor/ghosttykit}"
...
ln -sfn "$CACHE_XCFRAMEWORK" "$ROOT_XCFRAMEWORK"

echo "==> GhosttyKit ready at $ROOT_XCFRAMEWORK"
