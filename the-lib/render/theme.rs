...
fn base16_theme_data() -> &'static Value {
  BASE16_DEFAULT_THEME_DATA
    .get_or_init(|| load_embedded_theme(include_str!("../base16_theme.toml"), "base16_default"))
}
...
pub fn base16_default_theme() -> &'static Theme {
  BASE16_DEFAULT_THEME.get_or_init(|| {
    Theme {
...
pub struct Config {
  light:    String,
  dark:     String,
  /// A theme to choose when the terminal did not declare either light or dark
  /// mode. When not specified the dark theme is preferred.
...
impl Theme {
  ...
  pub fn scope(&self, highlight: Highlight) -> &str {
    self.scopes.get(highlight.idx()).map(String::as_str).unwrap_or("")
  }
