...
pub fn command_palette(cx: &mut Context) {
  ...
  impl CommandItem {
    fn name(&self) -> String {
      match self {
        CommandItem::Static { command, .. } => command.name().to_string(),
        CommandItem::Typable { command, .. } => format!(":{}", command.name),
      }
    }

    fn bindings(&self) -> &str {
      match self {
        CommandItem::Static { bindings, .. } | CommandItem::Typable { bindings, .. } => bindings,
      }
    }

    fn doc(&self) -> &str {
      match self {
        CommandItem::Static { command, .. } => command.doc(),
        CommandItem::Typable { command, .. } => command.doc,
      }
    }
  }

  // Build reverse keymap to get keybindings for each command
  let reverse_map: ReverseKeymap = cx.editor.keymaps.reverse_map();

  // Collect all static commands
  let mut items: Vec<CommandItem> = MappableCommand::STATIC_COMMAND_LIST
    .iter()
    .filter(|cmd| cmd.name() != "no_op")
    .map(|cmd| {
      let bindings = reverse_map
        .get(cmd.name())
        .map(|seqs| {
          seqs
            .iter()
  ...
  // Add typable commands from the command registry
  let typable_commands = cx.editor.command_registry.all_commands();
  for cmd in typable_commands {
    // Check if this typable command has a keybinding (via reverse keymap)
    // Typable commands can be bound too
    let bindings = reverse_map
      .get(cmd.name)
      .map(|seqs| {
        seqs
  ...
  let command_registry = cx.editor.command_registry.clone();

  // Action handler to execute selected command
  let action_handler =
    std::sync::Arc::new(move |item: &CommandItem, _: &(), _action: PickerAction| {
      let item = item.clone();
      let registry = command_registry.clone();

      crate::ui::job::dispatch_blocking(move |editor, compositor| {
        // Create a minimal jobs instance for command execution
        let mut jobs = crate::ui::job::Jobs::new();
        ...
        match item {
          CommandItem::Static { command, .. } => {
            command.execute(&mut ctx);
          },
          CommandItem::Typable { command, .. } => {
            // Execute typable command with empty arguments
            if let Err(e) = registry.execute(&mut ctx, command.name, "", PromptEvent::Validate) {
  ...
  let picker = Picker::new(columns, 0, items, (), |_| {})
    .with_action_handler(action_handler)
    .without_preview()
    .with_headers();
