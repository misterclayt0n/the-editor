...
fn test_dirty_after_print() {
  ...
  assert!(dirty_rows.contains(&0), "Row 0 should be dirty after print");

  // Clear dirty and verify
  term.clear_dirty();
  let dirty_rows = term.get_dirty_rows();
  assert!(dirty_rows.is_empty(), "No rows should be dirty after clear");
}
...
fn test_multiple_rows_dirty() {
  ...
  // Write to third row
  term.write(b"line3").expect("Failed to write to terminal");

  let dirty_rows = term.get_dirty_rows();

  // All three rows should be dirty
  assert!(dirty_rows.contains(&0), "Row 0 should be dirty (line1)");
  assert!(dirty_rows.contains(&1), "Row 1 should be dirty (line2)");
  assert!(dirty_rows.contains(&2), "Row 2 should be dirty (line3)");
}
...
fn test_incremental_dirty_tracking() {
  ...
  term.write(b"line2").expect("Failed to write to terminal");
  ...
  assert!(dirty_rows.contains(&1), "Row 1 should be dirty (new write)");
}
