...
pub use crate::core::diagnostics::LanguageServerId;
...
pub enum Error {
  ...
  Unhandled,
  #[error(transparent)]
  ExecutableNotFound(#[from] the_editor_stdx::env::ExecutableNotFoundError),
  #[error(transparent)]
...
pub mod util {
  ...
  ///
  /// Panics when [`pos_to_lsp_pos`] would for an invalid range on the
  /// diagnostic.
  pub fn diagnostic_to_lsp_diagnostic(
    doc: &Rope,
    diag: &crate::core::diagnostics::Diagnostic,
  ...
  pub fn lsp_pos_to_pos(
    ...
    // so we must use `line_end_char_index` instead of `doc.line_to_char(pos_line +
    // 1)`
    //
    // FIXME: the_editor does not fully comply with the LSP spec for line
    // terminators. The LSP standard requires that line terminators are ['\n',
    // '\r\n', '\r']. Without the unicode-linebreak feature disabled, the `\r`
    // terminator is not handled by the_editor. With the unicode-linebreak
    // feature, the_editor recognizes multiple extra line break chars
    // which means that positions will be decoded/encoded incorrectly in their
    // presence
  ...
  pub fn lsp_range_to_range(
    ...
    // ranges where end < start. They don't make much sense but vscode simply caps
  ...
  pub fn generate_transaction_from_edits(
    ...
    if edits.len() == 1 {
      ...
      if is_document_replacement {
        ...
        return crate::core::diff::compare_ropes(doc, &new_text);
      }
    }
...
pub enum MethodCall {
  WorkDoneProgressCreate(WorkDoneProgressCreateParams),
  ...
  UnregisterCapability(UnregistrationParams),
  ShowDocument(ShowDocumentParams),
}

impl MethodCall {
  pub fn parse(method: &str, params: jsonrpc::Params) -> Result<MethodCall> {
    use lsp::types::request::Request;
    let request = match method {
      the_editor_lsp_types::types::request::WorkDoneProgressCreate::METHOD => {
        let params: WorkDoneProgressCreateParams = params.parse()?;
        Self::WorkDoneProgressCreate(params)
      },
      the_editor_lsp_types::types::request::ApplyWorkspaceEdit::METHOD => {
        let params: ApplyWorkspaceEditParams = params.parse()?;
        Self::ApplyWorkspaceEdit(params)
      },
      the_editor_lsp_types::types::request::WorkspaceFoldersRequest::METHOD => {
        Self::WorkspaceFolders
      },
      the_editor_lsp_types::types::request::WorkspaceConfiguration::METHOD => {
        let params: ConfigurationParams = params.parse()?;
        Self::WorkspaceConfiguration(params)
      },
      the_editor_lsp_types::types::request::RegisterCapability::METHOD => {
        let params: RegistrationParams = params.parse()?;
        Self::RegisterCapability(params)
      },
      the_editor_lsp_types::types::request::UnregisterCapability::METHOD => {
        let params: UnregistrationParams = params.parse()?;
        Self::UnregisterCapability(params)
      },
      the_editor_lsp_types::types::request::ShowDocument::METHOD => {
        let params: ShowDocumentParams = params.parse()?;
...
pub enum Notification {
  ...
  ProgressMessage(ProgressParams),
}

impl Notification {
  pub fn parse(method: &str, params: jsonrpc::Params) -> Result<Notification> {
    use lsp::types::notification::Notification as _;

    let notification = match method {
      the_editor_lsp_types::types::notification::Initialized::METHOD => Self::Initialized,
      the_editor_lsp_types::types::notification::Exit::METHOD => Self::Exit,
      the_editor_lsp_types::types::notification::PublishDiagnostics::METHOD => {
        let params: PublishDiagnosticsParams = params.parse()?;
        Self::PublishDiagnostics(params)
      },

      the_editor_lsp_types::types::notification::ShowMessage::METHOD => {
        let params: ShowMessageParams = params.parse()?;
        Self::ShowMessage(params)
      },
      the_editor_lsp_types::types::notification::LogMessage::METHOD => {
        let params: LogMessageParams = params.parse()?;
        Self::LogMessage(params)
      },
      the_editor_lsp_types::types::notification::Progress::METHOD => {
        let params: ProgressParams = params.parse()?;
...
impl Registry {
  pub fn new(syn_loader: Arc<ArcSwap<crate::core::syntax::Loader>>) -> Self {
    Self {
  ...
  /// If this method is called, all documents that have a reference to the
  /// language server have to refresh their language servers,
  /// See the_editor_view::editor::Editor::refresh_language_servers
...
impl ProgressStatus {
  pub fn progress(&self) -> Option<&WorkDoneProgress> {
    match &self {
...
/// has a unique id assigned at creation through [`Registry`]. This id is then
/// used to store the progress in this map.
pub struct LspProgressMap(HashMap<LanguageServerId, HashMap<ProgressToken, ProgressStatus>>);
...
impl LspProgressMap {
  ...
  /// Returns a map of all tokens corresponding to the language server with
  /// `id`.
  pub fn progress_map(
    &self,
    id: LanguageServerId,
  ) -> Option<&HashMap<ProgressToken, ProgressStatus>> {
    self.0.get(&id)
  }
  ...
  pub fn is_created(&mut self, id: LanguageServerId, token: &ProgressToken) -> bool {
    self
      .0
      .get(&id)
  ...
  pub fn create(&mut self, id: LanguageServerId, token: ProgressToken) {
    self
      .0
      .entry(id)
      .or_default()
  ...
  /// Ends the progress by removing the `token` from server with `id`, if
  /// removed returns the value.
  pub fn end_progress(
    &mut self,
    id: LanguageServerId,
    token: &ProgressToken,
  ...
  /// Updates the progress of `token` for server with `id` to begin state
  /// `status`
  pub fn begin(
    &mut self,
    id: LanguageServerId,
...
/// start_client takes both a LanguageConfiguration and a
/// LanguageServerConfiguration to ensure that it is only called when it makes
/// sense.
fn start_client(
  id: LanguageServerId,
  name: String,
  ...
  let (workspace, workspace_is_cwd) = the_editor_loader::find_workspace();
...
/// * If no root marker and we stopped at a `root_dirs` entry, return the
///   directory we stopped at
/// * If we stopped at `workspace` instead and `workspace_is_cwd == false`
///   return `None`
/// * If we stopped at `workspace` instead and `workspace_is_cwd == true` return
///   `workspace`
pub fn find_lsp_workspace(
  ...
  let mut file = if file.is_absolute() {
    ...
    let current_dir = the_editor_stdx::env::current_working_dir();
  ...
  fn converts_lsp_pos_to_pos() {
    macro_rules! test_case {
      ($doc:expr,($x:expr, $y:expr) => $want:expr) => {
        ...
        let pos = the_editor_lsp_types::types::Position::new($x, $y);
