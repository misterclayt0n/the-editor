...
//! local but instead of being local to a single thread, the statics are local
//! to a single tokio-runtime. The implementation requires locking so it's not
//! exactly efficient.
//!
//! Therefore this function is only enabled during integration tests and behaves
//! like a normal static otherwise. I would prefer this module to be fully
//! private and to only export the macro but the macro still need to construct
//! these internals so it's marked `doc(hidden)` instead
