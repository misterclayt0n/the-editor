  ...
  fn inc_running(&self, generation: u32) {
    ...
    loop {
      ...
      let res =
        self
          .state
          .compare_exchange_weak(state, state.saturating_add(off), Relaxed, Relaxed);
      match res {
        Ok(_) => break,
...
///
/// It can be used to cancel async futures very efficiently but can also be
/// checked for cancelation very quickly (single atomic read) in blocking code.
...
impl TaskHandle {
  ...
  /// [`TaskController`]. Immediately returns if `cancel` was already called
  /// since
