...
impl DiffHandle {
  ...
  pub fn update_document(&self, doc: Rope, block: bool) -> bool {
    let lock = the_editor_event::lock_frame();
...
impl Diff<'_> {
  ...
  pub fn prev_hunk(&self, line: u32) -> Option<u32> {
    ...
    match res {
      // Search found a hunk that ends exactly at this line (so it does not include the current
      // line). We can usually just return that hunk, however a special case for empty hunk is
      // necessary which represents a pure removal.
      // Removals are technically empty but are still shown as single line hunks
      // and as such we must jump to the previous hunk (if it exists) if we are already inside the
      // removal
  ...
  ///
  /// Hunks are returned at most once even when intersecting with multiple of
  /// the line ranges.
