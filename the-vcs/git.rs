...
fn get_repo_dir(file: &Path) -> Result<&Path> {
    file.parent().ok_or_else(|| eyre!("file has no parent directory"))
}

pub fn get_diff_base(file: &Path) -> Result<Vec<u8>> {
    debug_assert!(!file.exists() || file.is_file());
    debug_assert!(file.is_absolute());
    let file = gix::path::realpath(file).wrap_err("resolve symlinks")?;

    // TODO cache repository lookup

    let repo_dir = get_repo_dir(&file)?;
    let repo = open_repo(repo_dir)
        .wrap_err("failed to open git repo")?
        .to_thread_local();
...
fn status(repo: &Repository, f: impl Fn(Result<FileChange>) -> bool) -> Result<()> {
    let work_dir = repo
        .workdir()
        .ok_or_else(|| eyre!("working tree not found"))?
        .to_path_buf();
...
fn find_file_in_commit(repo: &Repository, commit: &Commit, file: &Path) -> Result<ObjectId> {
    let repo_dir = repo.workdir().ok_or_else(|| eyre!("repo has no worktree"))?;
    let rel_path = file.strip_prefix(repo_dir)?;
    let tree = commit.tree()?;
    let tree_entry = tree
        .lookup_entry_by_path(rel_path)?
        .ok_or_else(|| eyre!("file is untracked"))?;
