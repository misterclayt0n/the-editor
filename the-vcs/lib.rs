//! `the_editor_vcs` provides types for working with diffs from a Version
//! Control System (VCS). Currently `git` is the only supported provider for
//! diffs, but this architecture allows for other providers to be added in the
//! future.
//!
...
/// features. Currently only `git` is supported.
#[derive(Clone)]
