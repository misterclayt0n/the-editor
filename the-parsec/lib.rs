...
// `sym` (trait implementation for `&'static str`), `map`, `pred` (filter),
// `one_or_more`, `zero_or_more`, as well as the `Parser` trait originate mostly
// from that post. The remaining parsers and parser combinators are either based
// on https://github.com/archseer/snippets.nvim/blob/a583da6ef130d2a4888510afd8c4e5ffd62d0dce/lua/snippet/parser.lua#L5-L138
// or are novel.
...
///
/// This parser succeeds if the next characters in the input are equal to the
/// given string literal.
///
/// Note that [str::parse] interferes with calling [Parser::parse] on string
/// literals directly; this trait implementation works when used within any
/// parser combinator but does not work on its own. To call [Parser::parse] on a
/// parser for a string literal, use the [token] parser.
///
/// # Examples
///
/// ```
...
/// and is only necessary to avoid conflict with [str::parse]. See the
/// documentation for the `&'static str` implementation of [Parser].
///
/// # Examples
///
/// ```
...
/// A parser which matches all values until the specified pattern no longer
/// match.
///
...
/// A parser combinator which matches a sequence of parsers in an all-or-nothing
/// fashion.
///
/// The returned value is a tuple containing the outputs of all parsers in
/// order. Each parser in the sequence may be typed differently.
///
/// # Examples
///
/// ```
...
// Seq is implemented using trait-implementations of Parser for various size
// tuples. This allows sequences to be typed heterogeneously.
...
/// A parser combinator which chooses the first of the input parsers which
/// matches successfully.
///
/// All input parsers must have the same output type. This is a variadic form
/// for [or].
///
/// # Examples
///
...
/// A parser combinator which takes a parser as input and maps the output using
/// the given transformation function.
///
/// This corresponds to [Result::map]. The value is only mapped if the input
/// parser matches against input.
///
/// # Examples
///
/// ```
...
/// A parser combinator which succeeds if the first given parser matches the
/// input and the second given parse also matches.
///
/// # Examples
///
/// ```
...
///
/// The parser produced with this combinator always succeeds. If the given
/// parser succeeds, `Some(value)` is returned where `value` is the output of
/// the given parser. Otherwise, `None`.
///
/// # Examples
///
/// ```
