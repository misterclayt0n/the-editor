...
//! own vtable implementation. Otherwise implementing the event system would not
//! be possible. A nice bonus of this approach is that we can optimize the
//! vtable a bit more. Normally a dyn Trait fat pointer contains two pointers: A
//! pointer to the data itself and a pointer to a global (static) vtable entry
//! which itself contains multiple other pointers (the various functions of the
//! trait, drop, size and align). That makes dynamic dispatch pretty slow
//! (double pointer indirections). However, we only have a single function
//! in the hook trait and don't need a drop implementation (event system is
...
///
/// If extern types were stable, this could be implemented as `extern { pub type
/// Opaque; }` but until then we can use this.
///
/// Care should be taken that we don't use a concrete instance of this. It
/// should only be used through a reference, so we can maintain something else's
/// lifetime.
struct Opaque(());
...
impl ErasedHook {
  pub(crate) fn new_dynamic<H: Fn() -> Result<()> + 'static + Send + Sync>(hook: H) -> ErasedHook {
