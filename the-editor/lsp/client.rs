...
impl Client {
  pub fn try_add_doc(
    ...
    // this server definitely doesn't support multiple workspace, no need to check
    // capabilities
    ...
    let Some(capabilities) = self.capabilities.get() else {
      ...
      // for an LSP that **doesn't support workspace_folders** before initaliation is
      // finished the new roots are ignored.
      // That particular edgecase would require retroactively spawning new LSP
      // clients and therefore also require us to retroactively update the
      // corresponding documents LSP client handle. It's doable but a pretty
      // weird edgecase so let's wait and see if anyone ever runs into it.
  ...
  fn add_workspace_folder(
    ...
    // No need to add a new workspace root here as there is no logical root for this
    // file let the server deal with this
  ...
  pub fn start(
    ...
    let cmd = the_editor_stdx::env::which(cmd)?;
    ...
    // TODO: do we need bufreader/writer here? or do we use async wrappers on
    // unblock?
  ...
  pub fn supports_feature(&self, feature: LanguageServerFeature) -> bool {
    ...
    match feature {
      ...
      LanguageServerFeature::CodeAction => {
        matches!(
          capabilities.code_action_provider,
          Some(
            CodeActionProviderCapability::Simple(true) | CodeActionProviderCapability::Options(_),
          )
        )
      },
    ...
    R::Params: serde::Serialize,
  {
    ...
    // It's important that this is not part of the future so that it gets executed
    // right away and the request order stays consistent.
  ...
  pub fn will_rename<'a>(
    &'a self,
    old_path: &'a Path,
    new_path: &'a Path,
    is_dir: bool,
  ) -> Option<impl Future<Output = Result<Option<lsp::WorkspaceEdit>>> + 'a> {
    ...
    let params = Box::new(lsp::RenameFilesParams { files });
    Some(async move {
      self
        .call_with_timeout::<lsp::request::WillRenameFiles>(&*params, 5)
        .await
    })
  }
  ...
  pub fn text_document_document_color(
    ...
    let params = lsp::DocumentColorParams {
      ...
      partial_result_params: the_editor_lsp_types::types::PartialResultParams {
