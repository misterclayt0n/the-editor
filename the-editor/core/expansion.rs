...
fn expand_variable(editor: &Editor, variable: Variable) -> Result<Cow<'static, str>> {
    ...
    match variable {
        ...
        Variable::CurrentWorkingDirectory => Ok(Cow::Owned(
            current_working_dir().to_string_lossy().to_string(),
        )),
        Variable::WorkspaceDirectory => Ok(Cow::Owned(
            find_workspace().0.to_string_lossy().to_string(),
        )),
...
mod tests {
    ...
    fn test_variable_from_name() {
        assert_eq!(Variable::from_name("cursor_line"), Some(Variable::CursorLine));
        assert_eq!(Variable::from_name("cursor_column"), Some(Variable::CursorColumn));
        assert_eq!(Variable::from_name("buffer_name"), Some(Variable::BufferName));
        assert_eq!(Variable::from_name("line_ending"), Some(Variable::LineEnding));
        assert_eq!(Variable::from_name("current_working_directory"), Some(Variable::CurrentWorkingDirectory));
        assert_eq!(Variable::from_name("workspace_directory"), Some(Variable::WorkspaceDirectory));
        ...
        assert_eq!(Variable::from_name("selection_line_start"), Some(Variable::SelectionLineStart));
        assert_eq!(Variable::from_name("selection_line_end"), Some(Variable::SelectionLineEnd));
    ...
    fn test_variable_as_str() {
        ...
        assert_eq!(Variable::CurrentWorkingDirectory.as_str(), "current_working_directory");
        ...
        assert_eq!(Variable::SelectionLineStart.as_str(), "selection_line_start");
        assert_eq!(Variable::SelectionLineEnd.as_str(), "selection_line_end");
    }

    #[tokio::test]
    async fn test_expand_unicode() {
        use crate::core::command_line::{TokenKind, ExpansionKind, Token};
