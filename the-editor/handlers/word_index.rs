    ...
    match event {
      ...
      Event::Update(doc, change) => {
        if let Some(pending_change) = self.changes.get_mut(&doc) {
          ...
          pending_change.changes = mem::take(&mut pending_change.changes).compose(change.changes);
      ...
      Event::Delete(doc, text) => {
        // If there are pending changes that haven't been indexed since the last
        // debounce, forget them and delete the old text.
...
struct WordIndexInner {
  ...
  ///
  /// Words are very likely to be reused many times. Instead of storing
  /// duplicates we keep a reference count of times a word is used. When the
  /// reference count drops to zero the word is removed from the index.
...
impl WordIndex {
  ...
  ///
  /// This task wraps a MPSC queue and spawns blocking tasks which update the
  /// index. Updates are applied one-by-one to ensure that changes to the
  /// index are **serialized**: updates to each document must be applied
  /// in-order.
...
///
/// The window is larger than the changed area and can encompass multiple
/// insert/delete operations if they are grouped closely together.
///
/// The ranges of the old and new text should usually be of different sizes. For
/// example a deletion of "foo" surrounded by large retain sections would give a
/// longer window into the `old_text` and shorter window of `new_text`.
/// Vice-versa for an insertion. A full replacement of a word though would give
/// two slices of the same size.
fn changed_windows<'a>(
  ...
  iter::from_fn(move || {
    loop {
      ...
      let old_window =
        old_start.saturating_sub(MAX_WORD_LEN)..(old_pos + MAX_WORD_LEN).min(old_text.len_chars());
      let new_window =
        new_start.saturating_sub(MAX_WORD_LEN)..(new_pos + MAX_WORD_LEN).min(new_text.len_chars());
...
pub(crate) fn register_hooks(handlers: &Handlers) {
  ...
  register_hook!(move |event: &mut DocumentDidClose<'_>| {
    if event.doc.word_completion_enabled() {
      the_editor_event::send_blocking(&tx, Event::Delete(event.doc.id(), event.doc.text().clone()));
    }
    Ok(())
  });
...
mod tests {
  ...
  use super::*;
  use crate::core::diff::compare_ropes;
    ...
    I: IntoIterator<Item = S>,
  {
    ...
    let expect_removed: HashSet<_> = expect_removed.into_iter().map(|i| i.to_string()).collect();
    let expect_inserted: HashSet<_> = expect_inserted.into_iter().map(|i| i.to_string()).collect();
