...
impl CommandBatcher {
    ...
    pub fn add_command(&mut self, command: RenderCommand) {
        match command {
            RenderCommand::Rect { x, y, width, height, color } => {
                self.rect_batch.push(RectCommand { x, y, width, height, color });
            }
            ...
            RenderCommand::Selection { x, y, width, height, color } => {
                self.selection_batch.push(SelectionCommand { x, y, width, height, color });
            }
            RenderCommand::Cursor { x, y, width, height, color } => {
                // Only keep the latest cursor position
                self.cursor_batch = Some(CursorCommand { x, y, width, height, color });
            }
        }
    }
    ...
    pub fn execute(&mut self, renderer: &mut the_editor_renderer::Renderer) {
        ...
        if let Some(cursor) = &self.cursor_batch {
            renderer.draw_rect(cursor.x, cursor.y, cursor.width, cursor.height, cursor.color);
        }
