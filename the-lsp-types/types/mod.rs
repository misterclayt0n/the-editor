    ...
    (impl $typ: ident { $( $(#[$attr:meta])* pub const $name: ident : $enum_type: ty = $value: expr; )* }) => {
        ...
        impl std::fmt::Debug for $typ {
            fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                match *self {
                    $(
                    Self::$name => crate::types::fmt_pascal_case(f, stringify!($name)),
                    )*
        ...
        impl std::convert::TryFrom<&str> for $typ {
            ...
            fn try_from(value: &str) -> Result<Self, Self::Error> {
                match () {
                    $(
                        _ if {
                            const X: (crate::types::PascalCaseBuf, usize) = crate::types::fmt_pascal_case_const(stringify!($name));
                            let (buf, len) = X;
...
/// schema or pattern. Examples are a filter that applies to TypeScript files on
/// disk or a filter the applies to JSON files with name package.json:
///
    ...
    T: serde::Deserialize<'de>,
  {
    Ok(
      match Option::<Value>::deserialize(serializer).map_err(serde::de::Error::custom)? {
        ...
        Some(other) => Some(TagSupport::<T>::deserialize(other).map_err(serde::de::Error::custom)?),
        None => None,
      },
    )
  }
...
pub struct GeneralClientCapabilities {
  ...
  /// a request for which the client will not process the response anymore
...
/// properties of result literals like `CompletionItem` or
/// `SignatureInformation`. If the format is `markdown` the content should follow the [GitHub Flavored Markdown Specification](https://github.github.com/gfm/).
///
