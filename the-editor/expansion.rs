...
fn expand_variable(editor: &Editor, variable: Variable) -> Result<Cow<'static, str>> {
  ...
  match variable {
    ...
    Variable::CursorColumn => {
      ...
      let position = crate::core::position::coords_at_pos(text, cursor);
    ...
    Variable::BufferName => {
      ...
      if let Some(path) = doc.relative_path() {
        ...
        Ok(Cow::Borrowed(crate::core::document::SCRATCH_BUFFER_NAME))
      }
    },
    Variable::LineEnding => Ok(Cow::Borrowed(doc.line_ending.as_str())),
    Variable::CurrentWorkingDirectory => {
      Ok(std::borrow::Cow::Owned(
        the_editor_stdx::env::current_working_dir()
          .to_string_lossy()
          .to_string(),
      ))
    },
    Variable::WorkspaceDirectory => {
      Ok(std::borrow::Cow::Owned(
        the_editor_loader::find_workspace()
          .0
          .to_string_lossy()
          .to_string(),
      ))
