...
use the_editor_renderer::Renderer;
...
pub trait Component: Any + AnyComponent {
  ...
  /// Should redraw? Useful for saving redraw cycles if we know component didn't
  /// change.
  ...
  /// viewport is the maximum allowed area, and the child should stay within
  /// those bounds.
  ///
  /// The returned size might be larger than the viewport if the child is too
  /// big to fit. In this case the parent can use the values to calculate
  /// scroll.
...
impl Compositor {
  ...
  pub fn push(&mut self, mut layer: Box<dyn Component>) {
    // Clear last_picker if this is a picker component
    // TODO: Add picker support
  ...
  pub fn handle_event(&mut self, event: &Event, cx: &mut Context) -> bool {
    // If it is a key event, a macro is being recorded, and a macro isn't being
    // replayed, push the key event to the recording.
    ...
    // propagate events through the layers until we either find a layer that
    // consumes it or we run out of layers (event bubbling), starting at the
    // front layer and then moving to the background.
