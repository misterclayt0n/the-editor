...
fn regex<'a>() -> impl Parser<'a, Output = Transform> {
  map(
    ...
    |(_, value, _, replacement, _, options)| {
      Transform {
        regex: value,
        replacement,
        options,
...
mod test {
  ...
  fn parse_placeholders_in_function_call() {
    ...
    // The `$0` tabstop should not have placeholder text. The parser should handle
    // this case normally and then the placeholder text should be discarded
    // during elaboration.
    assert_eq!(
      Ok(vec![
        Text("sizeof(".into()),
  ...
  fn parse_tabstop_nested_in_placeholder() {
    assert_eq!(
      Ok(vec![Placeholder {
        tabstop: 1,
        value:   vec![Text("var, ".into()), Tabstop {
          tabstop:   2,
          transform: None,
        }],
      }]),
  ...
  fn parse_placeholder_nested_in_placeholder() {
    assert_eq!(
      Ok({
        vec![Placeholder {
          tabstop: 1,
          value:   vec![Text("foo ".into()), Placeholder {
            tabstop: 2,
  ...
  fn parse_variable() {
    ...
    assert_snippet("far$farboo", &[Text("far".into()), Variable {
      name:      "farboo".into(),
      transform: None,
      default:   None,
    }]);
    ...
    assert_snippet("$123", &[Tabstop {
      tabstop:   123,
      transform: None,
    }]);
    assert_snippet("$farboo", &[Variable {
      name:      "farboo".into(),
      transform: None,
      default:   None,
    }]);
    assert_snippet("$far12boo", &[Variable {
      name:      "far12boo".into(),
      transform: None,
      default:   None,
    }]);
    assert_snippet("000_${far}_000", &[
      Text("000_".into()),
      Variable {
        name:      "far".into(),
        transform: None,
  ...
  fn parse_variable_transform() {
    ...
    assert_snippet("${foo/regex/format/gmi}", &[Variable {
      name:      "foo".into(),
      transform: Some(Transform {
        regex:       "regex".into(),
    ...
    assert_snippet("${foo/([A-Z][a-z])/format/}", &[Variable {
      name:      "foo".into(),
      transform: Some(Transform {
        regex:       "([A-Z][a-z])".into(),
        replacement: vec![FormatItem::Text("format".into())],
    ...
    // tricky regex
    assert_snippet("${foo/m\\/atch/$1/i}", &[Variable {
      name:      "foo".into(),
      transform: Some(Transform {
        regex:       "m/atch".into(),
    ...
    // format string
    assert_snippet("${foo/.*/${0:fooo}/i}", &[Variable {
      name:      "foo".into(),
      transform: Some(Transform {
        regex:       ".*".into(),
    ...
    assert_snippet("${foo/.*/${1}/i}", &[Variable {
      name:      "foo".into(),
      transform: Some(Transform {
        regex:       ".*".into(),
        replacement: vec![FormatItem::Capture(1)],
    ...
    assert_snippet("${foo/.*/This-$1-encloses/i}", &[Variable {
      name:      "foo".into(),
      transform: Some(Transform {
        regex:       ".*".into(),
        replacement: vec![
    ...
    assert_snippet("${foo/.*/complex${1:else}/i}", &[Variable {
      name:      "foo".into(),
      transform: Some(Transform {
        regex:       ".*".into(),
        replacement: vec![
    ...
    assert_snippet("${foo/.*/complex${1:+if}/i}", &[Variable {
      name:      "foo".into(),
      transform: Some(Transform {
        regex:       ".*".into(),
        replacement: vec![
    ...
    assert_snippet("${foo/.*/complex${1:?if:else}/i}", &[Variable {
      name:      "foo".into(),
      transform: Some(Transform {
        regex:       ".*".into(),
        replacement: vec![
    ...
    assert_snippet("${foo/.*/complex${1:/upcase}/i}", &[Variable {
      name:      "foo".into(),
      transform: Some(Transform {
        regex:       ".*".into(),
        replacement: vec![
    ...
    assert_snippet("${TM_DIRECTORY/src\\//$1/}", &[Variable {
    ...
    assert_snippet("${TM_SELECTED_TEXT/a/\\/$1/g}", &[Variable {
    ...
    assert_snippet("${TM_SELECTED_TEXT/a/in\\/$1ner/g}", &[Variable {
      name:      "TM_SELECTED_TEXT".into(),
      transform: Some(Transform {
        regex:       "a".into(),
        replacement: vec![
    ...
    assert_snippet("${TM_SELECTED_TEXT/a/end\\//g}", &[Variable {
      name:      "TM_SELECTED_TEXT".into(),
      transform: Some(Transform {
        regex:       "a".into(),
