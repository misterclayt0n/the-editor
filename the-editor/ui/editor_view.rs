use crate::{
    core::{
        commands::{self, OnKeyCallback, OnKeyCallbackKind, MappableCommand},
        doc_formatter::{DocumentFormatter, GraphemeSource},
        graphics::{CursorKind, Rect},
        grapheme::Grapheme,
        position::{char_idx_at_visual_offset, visual_offset_from_block, Position},
    },
    editor::Editor,
    keymap::{KeyBinding, KeymapResult, Keymaps, Mode},
    ui::compositor::{Callback, Component, Context, Event, EventResult, Surface},
};
...
impl Component for EditorView {
    fn handle_event(&mut self, event: &Event, cx: &mut Context) -> EventResult {
        match event {
            Event::Key(key) => {
                ...
                use crate::keymap::{Command, KeymapResult};
    ...
    fn render(&mut self, _area: Rect, renderer: &mut Surface, cx: &mut Context) {
        ...
        let content_rows = ((available_height / (font_size + LINE_SPACING)).floor().max(1.0)) as u16;
        ...
        while let Some(g) = formatter.next() {
            ...
            if rel_row != current_row {
                if !run_text.is_empty() {
                    flush_text_run(renderer, &mut run_text, run_start_x, run_y, font_size, run_color);
                }
                current_row = rel_row;
            }
