...
- Provider: `the-vcs::DiffProviderRegistry` (`jj` and `git` CLI backends, feature-gated).
...
1. **File path set/opened**
   - Client resolves a provider-specific diff base (`git HEAD:<path>`, `jj @-:<path>`).
...
- VCS backends currently shell out to CLI tools (`jj` / `git`) for portability in this workspace.
