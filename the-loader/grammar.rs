...
fn build_grammar(grammar: GrammarConfiguration, target: Option<&str>) -> Result<BuildStatus> {
  ...
  let grammar_dir_entries = grammar_dir.read_dir().with_context(|| {
    format!(
      "Failed to read directory {:?}. Did you use 'the-editor --grammar fetch'?",
      grammar_dir
    )
  })?;
  ...
  if grammar_dir_entries.count() == 0 {
    return Err(eyre!(
      "Directory {:?} is empty. Did you use 'the-editor --grammar fetch'?",
      grammar_dir
    ));
  };
...
fn build_tree_sitter_library(
  ...
  // used to delay dropping the temporary object file until after the compilation
  // is complete
  let _path_guard;
