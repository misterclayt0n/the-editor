...
pub struct LanguageConfiguration {
  #[serde(flatten)]
  pub syntax:   SyntaxLanguageConfig,
  #[serde(default, flatten)]
  pub services: LanguageServicesConfig,
}

impl LanguageConfiguration {
  pub fn language_id(&self) -> &str {
    &self.syntax.language_id
  }
}
...
pub enum IndentationHeuristic {
  ...
  Simple,
  /// Use tree-sitter indent queries to compute the expected absolute
  /// indentation level of the new line.
  TreeSitter,
  /// Use tree-sitter indent queries to compute the expected difference in
  /// indentation between the new line and the line before. Add this to the
  /// actual indentation level of the line before.
  #[default]
  Hybrid,
}
...
pub enum AutoPairConfig {
  /// Enables or disables auto pairing. False means disabled. True means to use
  /// the default pairs.
  Enable(bool),
...
pub struct SoftWrap {
  ...
  // NOTE: Option on purpose because the struct is shared between language config and global
  // config. By default the option is None so that the language config falls back to the global
  // config unless explicitly set.
  pub enable: Option<bool>,
  /// Maximum space left free at the end of the line.
  /// This space is used to wrap text at word boundaries. If that is not
  /// possible within this limit the word is simply split at the end of the
  /// line.
  ///
  /// This is automatically hard-limited to a quarter of the viewport to ensure
