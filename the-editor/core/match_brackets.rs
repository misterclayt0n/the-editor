...
fn find_pair(
  ...
  loop {
    ...
    if let Some((start_char, end_char)) = as_open_pair(doc, &node) {
      if let Some(pair_end) = find_pair_end(doc, node.next_sibling(), start_char, end_char, Forward)
      {
    ...
    if traverse_parents {
        ...
        iter::successors(node.next_sibling(), |node| node.next_sibling()).take(MATCH_LIMIT)
      {
        ...
        if find_pair_end(doc, sibling.prev_sibling(), start_char, end_char, Backward).is_some() {
