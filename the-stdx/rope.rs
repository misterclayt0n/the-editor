...
pub trait RopeSliceExt<'a>: Sized {
  ...
  /// Finds the closest byte index not exceeding `byte_idx` which lies on a
  /// character boundary.
  ///
  /// If `byte_idx` already lies on a character boundary then it is returned
  /// as-is. When `byte_idx` lies between two character boundaries, this
  /// function returns the byte index of the lesser / earlier / left-hand-side
  /// boundary.
  ///
  /// # Example
  ///
  ...
  /// Finds the closest byte index not below `byte_idx` which lies on a
  /// character boundary.
  ///
  /// If `byte_idx` already lies on a character boundary then it is returned
  /// as-is. When `byte_idx` lies between two character boundaries, this
  /// function returns the byte index of the greater / later / right-hand-side
  /// boundary.
  ///
  /// # Example
  ///
  ...
  ///
  /// If `byte_idx` already lies on a grapheme cluster boundary then it is
  /// returned as-is. When `byte_idx` lies between two grapheme cluster
  /// boundaries, this function returns the byte index of the lesser / earlier
  ...
  /// If `byte_idx` already lies on a grapheme cluster boundary then it is
  /// returned as-is. When `byte_idx` lies between two grapheme cluster
  /// boundaries, this function returns the byte index of the greater / later
  ...
  ///
  /// The returned iterator starts at the end of the slice and ends at the
  ...
  /// Returns an iterator over the grapheme clusters in the slice at the given
  /// byte index.
  ///
  /// # Example
  ///
  ...
  /// Returns an iterator over the grapheme clusters in a rope and the byte
  /// index where each grapheme cluster starts.
  ///
  /// # Example
  ///
  /// ```
  ...
  ///   (27, "🖼️".into())
  /// ]);
  /// let graphemes: Vec<_> = slice
  ///   .grapheme_indices_at(slice.len_bytes())
  ///   .reversed()
  ///   .collect();
  /// assert_eq!(graphemes.as_slice(), &[
  ///   (27, "🖼️".into()),
  ...
  ///
  /// If the byte index lies on the last grapheme cluster in the slice then this
  ...
  /// assert_eq!(
  ///   slice.next_grapheme_boundary(byte_idx + 1),
  ///   slice.next_grapheme_boundary(byte_idx)
  /// );
  /// assert_eq!(
  ///   slice.next_grapheme_boundary(byte_idx + 2),
  ///   slice.next_grapheme_boundary(byte_idx)
  /// );
  ///
  /// byte_idx = slice.next_grapheme_boundary(byte_idx);
  ...
  /// Finds the byte index of the `n`th grapheme cluster after the given
  /// `byte_idx`.
  ///
  /// If there are fewer than `n` grapheme clusters after `byte_idx` in the rope
  /// then this function returns `RopeSlice::len_bytes`.
  ///
  /// This is functionally equivalent to calling `next_grapheme_boundary` `n`
  /// times but is more efficient.
  ...
  ///
  /// If the byte index lies on the first grapheme cluster in the slice then
  ...
  /// assert_eq!(
  ///   slice.prev_grapheme_boundary(byte_idx - 1),
  ///   slice.prev_grapheme_boundary(byte_idx)
  /// );
  /// assert_eq!(
  ///   slice.prev_grapheme_boundary(byte_idx - 2),
  ///   slice.prev_grapheme_boundary(byte_idx)
  /// );
  ///
  /// byte_idx = slice.prev_grapheme_boundary(byte_idx);
  ...
  /// Finds the byte index of the `n`th grapheme cluster before the given
  /// `byte_idx`.
  ///
  /// If there are fewer than `n` grapheme clusters before `byte_idx` in the
  /// rope then this function returns zero.
  ///
  /// This is functionally equivalent to calling `prev_grapheme_boundary` `n`
  /// times but is more efficient.
...
impl<'a> RopeSliceExt<'a> for RopeSlice<'a> {
  ...
  fn regex_input_at_bytes<R: RangeBounds<usize>>(
    ...
    let input = match byte_range.start_bound() {
      Bound::Included(&pos) | Bound::Excluded(&pos) => RegexInput::new(RopeyCursor::at(self, pos)),
  ...
  fn floor_grapheme_boundary(self, mut byte_idx: usize) -> usize {
    ...
    let (mut chunk, mut chunk_byte_idx, ..) = self.chunk_at_byte(byte_idx);
    ...
    loop {
      match cursor.prev_boundary(chunk, chunk_byte_idx) {
        ...
        Err(GraphemeIncomplete::PrevChunk) => {
          let (ch, ch_byte_idx, ..) = self.chunk_at_byte(chunk_byte_idx - 1);
          chunk = ch;
  ...
  fn is_grapheme_boundary(self, byte_idx: usize) -> bool {
    // The byte must lie on a character boundary to lie on a grapheme cluster
    // boundary.
    ...
    loop {
      match cursor.is_boundary(chunk, chunk_byte_idx) {
        Ok(n) => return n,
        Err(GraphemeIncomplete::PreContext(n)) => {
          let (ctx_chunk, ctx_byte_start, ..) = self.chunk_at_byte(n - 1);
    ...
    for _ in 0..n {
      loop {
        match gc.next_boundary(chunk, chunk_byte_idx) {
          ...
          Err(GraphemeIncomplete::NextChunk) => {
            chunk_byte_idx += chunk.len();
            let (a, ..) = self.chunk_at_byte(chunk_byte_idx);
            chunk = a;
          },
          Err(GraphemeIncomplete::PreContext(n)) => {
            let ctx_chunk = self.chunk_at_byte(n - 1).0;
    ...
    for _ in 0..n {
      loop {
        match gc.prev_boundary(chunk, chunk_byte_idx) {
          ...
          Err(GraphemeIncomplete::PrevChunk) => {
            let (a, b, ..) = self.chunk_at_byte(chunk_byte_idx - 1);
            chunk = a;
            chunk_byte_idx = b;
          },
          Err(GraphemeIncomplete::PreContext(n)) => {
    ...
    loop {
        ...
        .next_boundary(self.current_chunk, self.chunk_byte_idx)
      {
        ...
        Err(GraphemeIncomplete::PreContext(idx)) => {
          let (chunk, byte_idx, ..) = self.text.chunk_at_byte(idx.saturating_sub(1));
...
/// An iterator over the grapheme clusters in a rope and the byte indices where
/// each grapheme cluster starts.
///
/// This iterator wraps `RopeGraphemes` and is also cursor-like. Use `reverse`
/// or `reversed` to toggle the direction of the iterator. See [RopeGraphemes].
#[derive(Debug, Clone)]
