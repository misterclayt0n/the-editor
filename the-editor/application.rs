use crate::{
    ...
    editor::Editor,
    input::InputHandler,
    keymap::{KeyBinding, Keymaps, Mode},
    ui::{
...
use the_editor_renderer::{Application, InputEvent, Renderer, KeyPress, Key};
...
impl App {
    pub fn new(mut editor: Editor) -> Self {
        // Create compositor with initial size
        let area = Rect::new(0, 0, 120, 40); // Default size, will be updated on resize
        let mut compositor = Compositor::new(area);

        // Get the current mode from editor
        let mode = editor.mode;

        // Create and push the main editor view
        let keymaps = Keymaps::default();
        let editor_view = Box::new(EditorView::new(keymaps));
        compositor.push(editor_view);

        // Add a test button (initially hidden, toggle with Ctrl+N)
        let button = Box::new(
            Button::new("Run")
                .with_rect(Rect::new(100, 1, 8, 2)) // Position at top-right
...
impl Application for App {
    fn init(&mut self, renderer: &mut Renderer) {
        ...
        renderer.set_ligature_protection(true);

        // Optional: allow users to specify a font file path via env var
        if let Ok(path) = std::env::var("THE_EDITOR_FONT_FILE") {
            if let Err(err) = renderer.configure_font_from_path(&path, 22.0) { // TODO: Get from editor config
    ...
    fn render(&mut self, renderer: &mut Renderer) {
        ...
        // We just need to draw our content
    ...
    fn handle_event(&mut self, event: InputEvent, _renderer: &mut Renderer) -> bool {
        ...
        let pending_char = self.compositor.layers.iter()
            .any(|layer| {
