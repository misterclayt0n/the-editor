...
use gix::{
  ...
  diff::Rewrites,
  dir::entry::Status,
  filter::plumbing::driver::apply::Delay,
  objs::tree::EntryKind,
...
pub fn get_workspace_root(file: &Path) -> Option<std::path::PathBuf> {
  ...
  let repo_dir = get_repo_dir(&file).ok()?;
...
fn open_repo(path: &Path) -> Result<ThreadSafeRepository> {
  ...
  // On windows various configuration options are bundled as part of the
  // installations This path depends on the install location of git and
  // therefore requires some overhead to lookup This is basically only used on
