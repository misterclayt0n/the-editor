...
pub fn read_query(lang: &str, query_filename: &str) -> String {
  tree_house::read_query(lang, |language| {
    the_editor_loader::grammar::load_runtime_file(language, query_filename).unwrap_or_default()
  })
}
...
impl Loader {
  ...
  pub fn language_for_name(&self, name: impl PartialEq<String>) -> Option<Language> {
    self
      ...
      .iter()
      .enumerate()
      .find_map(|(idx, config)| (name == config.config.language_id).then_some(Language(idx as u32)))
  }
  ...
  pub fn language_for_scope(&self, scope: &str) -> Option<Language> {
    self
      ...
      .iter()
      .enumerate()
      .find_map(|(idx, config)| (scope == config.config.scope).then_some(Language(idx as u32)))
  }
  ...
  pub fn language_for_shebang(&self, text: RopeSlice) -> Option<Language> {
    ...
    use the_editor_stdx::rope::Regex;
