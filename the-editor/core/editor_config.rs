...
//!
//! EditorConfig is an editor-agnostic format for specifying configuration in an
//! INI-like, human friendly syntax in `.editorconfig` files (which are intended
//! to be checked into VCS). This module provides functions to search for all
//! `.editorconfig` files that apply to a given path and returns an
//! `EditorConfig` type containing any specified configuration options.
//!
...
impl EditorConfig {
  /// Finds any configuration in `.editorconfig` files which applies to the
  /// given path.
  ///
  ...
  pub fn find(path: &Path) -> Self {
    ...
    // `path` are applied last and overwrite settings in files closer to the
    // search ceiling.
    //
    // > If multiple EditorConfig files have matching sections, the pairs from the
    // > closer
...
impl FromStr for Ini {
  ...
  fn from_str(source: &str) -> Result<Self, Self::Err> {
    ...
    // > EditorConfig files are in an INI-like file format. To read an EditorConfig
    // > file, take
...
fn is_glob_relative(source: &str) -> bool {
  // > If the glob contains a path separator (a `/` not inside square brackets),
  // > then the
  // > glob is relative to the directory level of the particular `.editorconfig`
  // > file itself.
  let mut idx = 0;
