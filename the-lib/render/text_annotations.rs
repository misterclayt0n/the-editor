...
use crate::{
  Tendril, position::Position, render::doc_formatter::FormattedGrapheme, syntax::{Highlight, OverlayHighlights}
};
...
pub trait LineAnnotation {
  ...
  /// `LineAnnotation`. In this case the line decorations  **must** ensure
  /// that virtual text anchored within that char range is skipped.
  ///
  /// # Returns
  ///
  /// The `char_idx` of the next anchor this `LineAnnotation` is interested in,
  ...
  /// Process an anchor (horizontal position is provided) and returns the next
  /// anchor.
  ///
  /// # Returns
  ///
  ...
  /// from the start of block where the traversal starts.  This includes the
  /// offset from other `LineAnnotations`. This allows inline annotations to
  /// consider the height of the text and "align" two different documents
  /// (like for side by side diffs).  These annotations that want to "align"
  /// two documents should therefore be added last so that other virtual text
  /// is also considered while aligning
...
/// Safety: We store LineAnnotation in a NonNull pointer. This is necessary to
/// work around an unfortunate inconsistency in rusts variance system that
/// unnnecesarily makes the lifetime invariant if implemented with safe code.
/// This makes the DocFormatter API very cumbersome/basically impossible to work
/// with.
///
/// Normally object types `dyn Foo + 'a` are covariant so if we used `Box<dyn
/// LineAnnotation + 'a>` below everything would be alright. However we want to
/// use `Cell<Box<dyn LineAnnotation + 'a>>` to be able to call the mutable
/// function on `LineAnnotation`. The problem is that some types like `Cell`
/// make all their arguments invariant. This is important for soundness normally
/// for the same reasons that `&'a mut T` is invariant over `T` (see <https://doc.rust-lang.org/nomicon/subtyping.html>). However for `&'a mut` (`dyn Foo + 'b`)
/// there is a specical rule in the language to make `'b` covariant (otherwise
/// trait objects would be super annoying to use). See  <https://users.rust-lang.org/t/solved-variance-of-dyn-trait-a> for
/// why this is sound. Sadly that rule doesn't apply to `Cell<Box<(dyn Foo +
/// 'a)>` (or other invariant types like `UnsafeCell` or `*mut (dyn Foo + 'a)`).
///
...
/// that is covariant over `'a` (or in other words it's a raw pointer, as long
/// as we don't hand out references we are free to do whatever we want).
    ...
    for (next_anchor, layer) in &self.line_annotations {
      loop {
        match next_anchor.get().cmp(&grapheme.char_idx) {
          Ordering::Less => {
            next_anchor.set(unsafe { layer.get().skip_concealed_anchors(grapheme.char_idx) })
          },
          Ordering::Equal => next_anchor.set(unsafe { layer.get().process_anchor(grapheme) }),
