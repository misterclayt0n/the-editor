...
impl JumpList {
  ...
  /// This is necessary to ensure that changes to documents do not leave
  /// jump-list selections pointing to parts of the text which no longer
  /// exist.
...
impl View {
  ...
  //
  pub fn offset_coords_to_in_view(&self, doc: &Document, scrolloff: usize) -> Option<ViewPosition> {
  ...
  pub fn offset_coords_to_in_view_center<const CENTERING: bool>(
    ...
    let (new_anchor, at_top) = match off {
      ...
      Err(VisualOffsetError::PosBeforeAnchorRow) => (true, true),
      Err(VisualOffsetError::PosAfterMaxRow) => (true, false),
    };
  ...
  pub fn last_visual_line(&self, doc: &Document) -> usize {
    ...
    match pos {
      ...
      Err(VisualOffsetError::PosAfterMaxRow) => visual_height.saturating_sub(1),
      Err(VisualOffsetError::PosBeforeAnchorRow) => 0,
    }
  }
  ...
  /// Get the text annotations to display in the current view for the given
  /// document and theme.
  pub fn text_annotations<'a>(
    ...
    // Note: We can't use add_overlay for syntax highlighting - that's for grapheme
    // replacement ACP highlighting will be applied directly in the rendering
    // pipeline via OverlayHighlighter TODO: Implement proper ACP syntax
    ...
    }) = doc.inlay_hints.get(&self.id)
    {
      ...
      let parameter_style = theme.and_then(|t| t.find_highlight("ui.virtual.inlay-hint.parameter"));
      let other_style = theme.and_then(|t| t.find_highlight("ui.virtual.inlay-hint"));

      // Overlapping annotations are ignored apart from the first so the order here is
      // not random: types -> parameters -> others should hopefully be the
      // "correct" order for most use cases, with the padding coming before and
      // after as expected.
      text_annotations
  ...
  /// Returns a usize typed position in bounds of the text if found in this
  /// view, None if out of view.
