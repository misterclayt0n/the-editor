  ...
  async fn recv_server_message(
    ...
    loop {
      ...
      match parts {
        ...
        Some((..)) => {},
        None => {
          // Workaround: Some non-conformant language servers will output
          // logging and other garbage into the same stream as
          // JSON-RPC messages. This can also happen from shell scripts that
          // spawn the server. Skip such lines and log a warning.
    ...
    loop {
      ...
      .await
      {
        ...
        Err(err) => {
          ...
          // Hack: inject a terminated notification so we trigger code that needs to
          // happen after exit
          let notification =
