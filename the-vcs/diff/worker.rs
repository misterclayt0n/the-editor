...
impl DiffWorker {
  ...
  pub async fn run(mut self, mut diff_base: Rope, mut doc: Rope) {
    {
      ...
      // interner is dropped here -> not held across awaits
    }

    // ---- event loop ----
    while let Some(event) = self.channel.recv().await {
      // collect all pending changes; this can .await freely
      let (maybe_doc, maybe_base) = self.accumulate_events(event).await;

      // update our owned state; no `interner` alive during the await
      if let Some(new_base) = maybe_base {
        diff_base = new_base;
      }
      if let Some(new_doc) = maybe_doc {
        doc = new_doc;
      }

      // do the heavy, non-Send work *after* the awaits, then drop it again
      #[cfg(test)]
      {
        let interner = InternedRopeLines::new(diff_base.clone(), doc.clone());
        if let Some(lines) = interner.interned_lines() {
        ...
        self.apply_hunks(interner.diff_base(), interner.doc());
        // interner dropped before next await
      }
      ...
      tokio::task::block_in_place(|| {
        ...
        self.apply_hunks(interner.diff_base(), interner.doc());
        // interner dropped when the closure ends
      });
    }
  }

  /// update the hunks (used by the gutter) by replacing it with
  /// `self.new_hunks`. `self.new_hunks` is always empty after this function
  /// runs. To improve performance this function tries to reuse the allocation
  /// of the old diff previously stored in `self.line_diffs`
      ...
      Some(RenderLock {
        ...
        tokio::task::spawn(async move {
          let res = {
            ...
            timeout_at(timeout, notify_clone.notified()).await
          };
          // we either reached the timeout or the diff is finished, release the render
          // lock
          drop(lock);
          if res.is_ok() {
